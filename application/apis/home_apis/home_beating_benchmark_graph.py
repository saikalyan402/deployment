    ...
    def post(self):
        ...
        for dictionary in [curr_scheme_dict, prev_scheme_dict]:
            ...
            for chart_entry in compare_chart:
                ...
                if aum_quar[period]['aum']>=0 and curr_total_aum>0:
                    if counter==0:
                        chart_entry[status] = round((aum_quar[period]['aum']/curr_total_aum)*100,2)
                    else:
                        chart_entry[status] = round((aum_quar[period]['aum']/prev_total_aum)*100,2)
